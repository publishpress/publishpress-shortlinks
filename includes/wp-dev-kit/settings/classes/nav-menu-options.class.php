 ...
 *
 */
if (! class_exists('WPDK_Settings_Nav_Menu_Options')) {
    class WPDK_Settings_Nav_Menu_Options extends WPDK_Settings_Abstract
    {
        // constans
        public $unique = '';
        ...
        public function wp_edit_nav_menu_walker($class, $menu_id)
        {
            ...
            if (version_compare($wp_version, '5.4.0', '<')) {
                if (! class_exists('WPDK_Settings_Walker_Nav_Menu_Edit')) {
                    WPDK_Settings::include_plugin_file('functions/walker.php');
                }

                return 'WPDK_Settings_Walker_Nav_Menu_Edit';
            }
            ...
            foreach ($this->sections as $section) {
                ...
                if (! empty($section['fields'])) {
                    foreach ($section['fields'] as $field) {
                        ...
                        WPDK_Settings::field($field, $this->get_meta_value($menu_item_id, $field), $this->unique . '[' . $menu_item_id . ']', 'menu');
                    }
                }
