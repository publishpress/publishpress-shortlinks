...
if ( ! class_exists( 'WPDK_Settings' ) ) {
	class WPDK_Settings {
		...
		public function __construct() {
			...
			add_action( 'admin_enqueue_scripts', array( 'WPDK_Settings', 'add_admin_enqueue_scripts' ) );
			add_action( 'wp_enqueue_scripts', array( 'WPDK_Settings', 'add_typography_enqueue_styles' ), 80 );
			add_action( 'wp_head', array( 'WPDK_Settings', 'add_custom_css' ), 80 );
			add_filter( 'admin_body_class', array( 'WPDK_Settings', 'add_admin_body_class' ) );
		...
		public static function setup() {
			...
			$params = array();
			if ( class_exists( 'WPDK_Settings_Options' ) && ! empty( self::$args['admin_options'] ) ) {
				foreach ( self::$args['admin_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						WPDK_Settings_Options::instance( $key, $params );
			...
			$params = array();
			if ( class_exists( 'WPDK_Settings_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
				foreach ( self::$args['customize_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						WPDK_Settings_Customize_Options::instance( $key, $params );
			...
			$params = array();
			if ( class_exists( 'WPDK_Settings_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
				foreach ( self::$args['metabox_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						WPDK_Settings_Metabox::instance( $key, $params );
			...
			$params = array();
			if ( class_exists( 'WPDK_Settings_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
				foreach ( self::$args['nav_menu_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						WPDK_Settings_Nav_Menu_Options::instance( $key, $params );
			...
			$params = array();
			if ( class_exists( 'WPDK_Settings_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
				foreach ( self::$args['profile_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						WPDK_Settings_Profile_Options::instance( $key, $params );
			...
			if ( class_exists( 'WPDK_Settings_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
				...
				foreach ( self::$args['taxonomy_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						WPDK_Settings_Taxonomy_Options::instance( $key, $params );
			...
			if ( class_exists( 'WPDK_Settings_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
				...
				foreach ( self::$args['widget_options'] as $key => $value ) {
					if ( ! isset( self::$inited[ $key ] ) ) {
						...
						self::$inited[ $key ] = true;
						$wp_widget_factory->register( WPDK_Settings_Widget::instance( $key, $value ) );
			...
			$params = array();
			if ( class_exists( 'WPDK_Settings_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
				foreach ( self::$args['comment_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						WPDK_Settings_Comment_Metabox::instance( $key, $params );
			...
			$params = array();
			if ( class_exists( 'WPDK_Settings_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
				foreach ( self::$args['shortcode_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						WPDK_Settings_Shortcoder::instance( $key, $params );

					}
				}

				// Once editor setup for gutenberg and media buttons
				if ( class_exists( 'WPDK_Settings_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
					foreach ( self::$shortcode_instances as $instance ) {
						if ( ! empty( $instance['show_in_editor'] ) ) {
							WPDK_Settings_Shortcoder::once_editor_setup();
							break;
						}
					}
				}
		...
		public static function includes() {
			...
			if ( ! empty( $fields ) ) {
				foreach ( $fields as $field ) {
					if ( ! class_exists( 'WPDK_Settings_Field_' . $field ) && class_exists( 'WPDK_Settings_Fields' ) ) {
		...
		public static function add_admin_enqueue_scripts() {
			...
			if ( ! empty( self::$fields ) ) {
				foreach ( self::$fields as $field ) {
					if ( ! empty( $field['type'] ) ) {
						$classname = 'WPDK_Settings_Field_' . $field['type'];
